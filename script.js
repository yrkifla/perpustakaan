...
window.bacaBuku = async function(bookId, bookTitle, bookLink) {
    try {
        // 1. Tambah jumlah pembaca di database secara real-time
        const bookRef = doc(db, "books", bookId);
        await updateDoc(bookRef, {
            reads: increment(1)
        });

        // 2. Buka link web buku di tab baru browser
        if (bookLink && bookLink.startsWith("http")) {
            window.open(bookLink, "_blank");
        } else {
            alert(`Membaca buku: "${bookTitle}". (Link web belum ditambahkan di database).`);
        }
    } catch (error) {
